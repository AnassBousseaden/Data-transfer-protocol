...
int udt_init(char *receiver, int port, int side, udt_socket_t *p) {
  ...
  srand((unsigned int)time(
      NULL));  // for random number generation (used in udt_send)
  return UDT_SUCCESS;
}
...
void udt_close(udt_socket_t *p) { close(p->s); }

// randomly decide (with probability PROB_LOSS) if
// a packet must be "lost"
bool should_i_drop() { return (rand() % 100 < PROB_LOSS); }
...
int udt_send(udt_socket_t *p, char *buf, int size) {
  ...
  ret = sendto(p->s, buf, size, 0, (struct sockaddr *)&(p->recv_addr),
               sizeof(p->recv_addr));
  if (ret != size) {
    perror("UDT - sendto");
  }
...
int udt_recv(udt_socket_t *p, char *buf, int size) {
  ...
  if (p->side == UDT_RECEIVER) {
    ...
    ret =
        recvfrom(p->s, buf, size, 0, (struct sockaddr *)&(p->recv_addr), &len);
  } else {
