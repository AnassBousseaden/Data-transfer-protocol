...
#define PROB_LOSS 0  // must be between 0 and 99
...
typedef struct udt_socket {
  ...
  struct sockaddr_in recv_addr;  // the address of the other side. In the case
                                 // of the receiver, it will only be filled when
                                 // we call udt_recv for the first time
} udt_socket_t;
